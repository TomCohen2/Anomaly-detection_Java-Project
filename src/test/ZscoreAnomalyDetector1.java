...
public class ZscoreAnomalyDetector1 implements TimeSeriesAnomalyDetector{
	private static float threshold = 0.19f;
	float[][] TXtest;
	float[][] zScores;
	float[][] means;
	float[][] stddev;
	...
	public void learnNormal(TimeSeries ts) {
		...
		for(int i=0;i<ts.getNumOfFeatures();i++) {
			...
			for(int j=0;j<ts.data[0].length;j++) {
				zScores[i][j] = zFunc(ts.data[i][j], means[i][j], stddev[i][ts.getNumOfRows()-1]);
...
//		}
	...
	public List<AnomalyReport> detect(TimeSeries ts) {
		...
		for(int i=0;i<ts.getNumOfFeatures();i++) {
			...
			for(int j=0;j<ts.data[0].length;j++) {
				testzScores[i][j] = zFunc(ts.data[i][j], testMeans[i][j], testStddev[i][ts.getNumOfRows()-1]);
				if(Math.abs(TXtest[i][j] - testzScores[i][j]) - Math.abs(TXtest[i][j]-zScores[i][j]) > threshold){
