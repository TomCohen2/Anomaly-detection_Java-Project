...
public class ZscoreAnomalyDetectorV2 implements TimeSeriesAnomalyDetector{
	...
	public void learnNormal(TimeSeries ts) {
		...
		TXValues = new float[ts.getNumOfFeatures()][ts.data[0].length];
		for(int i=0;i<ts.getNumOfFeatures();i++) {
			...
			for(int j=0;j<ts.data[0].length;j++) {
				...
				checkWhatIsCurrentMax(zScores[i][j], i, j, ts);
			}
		}
	...
	public List<AnomalyReport> detect(TimeSeries ts) {
		...
		for(int i=0;i<ts.getNumOfFeatures();i++) {
			...
			for(int j=0;j<ts.data[0].length;j++) {
				...
				if(Math.abs(Math.abs(TXtest[i][j]-testzScores[i][j]) - (Math.abs(TXtest[i][j] - zScores[i][j]))) > threshold){
				//if(testzScores[i][j] > TXValues[i][j]) {
...
//		System.out.println(testMeans[24][2173]);
	...
	private void checkWhatIsCurrentMax(float f, int i, int j, TimeSeries ts) {
		if(j==0) {
			TXtest[i][j] = f;
			TXValues[i][j] = ts.getData()[i][j];
			return;
		}
		else {
			if(TXtest[i][j-1] < f) {
